...
\ee
where $p_{\rm  just noticeable}$, $p_{\rm obvious}$ and $p_{\rm dominant}$ are the fractions of users who indicated the bulge was ``just noticeable", ``obvious" or ``domninent" respectively. This provides a single number, which ranges from $B_{\rm avg} = 0.0$ for galaxies with no bulge component, to $B_{\rm avg} = 1.0$ for spiral galaxies with dominant bulges, although we note there is no aprior reason to select these co-efficients exactly. On the right hand side of Figure \ref{pitch} we plot this measure of bulge prominence from GZ2 against the SDSS r-band luminosity of bulges as measured by \citet{Simard2011}. Curiously this is the quantity in \citet{Simard2011} which correlates most strongly wiht GZ2 bulge prominence (\ie. not $B/T$).  The best fit trend is 
\be 
L_{\rm r,bulge} = 8.63\pm0.01{\rm [units]}  + (1.54\pm0.03 {\rm [units]}) B_{\rm avg}.
\ee
Some of the scatter in this plot will be caused by the bulge-disc only models of \citet{Simard2011} being fit to galaxies which also host bars. \citet{Kruk2018} demonstrated that in bulge+disc+bar decompositions, there was a stronger correlations of $B/T$ with Galaxy Zoo consensus classifications of bulge size. 
